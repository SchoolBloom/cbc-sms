...
CREATE OR REPLACE FUNCTION public.admit_transfer_learner(
  ...
  p_identifier_type TEXT,      -- 'UPI', 'KNEC', or 'BIRTH_CERT'
...
BEGIN
  ...
  IF p_identifier_type NOT IN ('UPI', 'KNEC', 'BIRTH_CERT') THEN
    RAISE EXCEPTION 'Invalid identifier type: %. Must be UPI, KNEC, or BIRTH_CERT.', p_identifier_type;
  END IF;
  ...
  IF p_identifier_type = 'UPI' THEN
    ...
    LIMIT 1;
  ELSIF p_identifier_type = 'KNEC' THEN
    SELECT id, full_name, school_id
    INTO v_learner_id, v_learner_name, v_origin_school_id
    FROM public.learners
  ...
  ELSE -- 'BIRTH_CERT'
    ...
    INTO v_learner_id, v_learner_name, v_origin_school_id
    FROM public.learners
    WHERE birth_certificate_number = p_identifier_value
    LIMIT 1;
  END IF;
